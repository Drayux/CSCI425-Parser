    ...
    def getRow(self, rowId):
        ...
        for x in range(self.colCount):
            ...
            if val < 0:
                ...
                val = 'lambda'  # TODO: Not sure what interface we're going for with lambda chars
            else:
                val = chr(val)
...
class TTable:
    ...
    def getRow(self, fromState):
        ...
        ret = []
        try:
            row = self.data[fromState]
        except IndexError:
            print(f"WARNING: Invalid row ID specifed in getRow({fromState})")
            return ret
        # Accumulate output
